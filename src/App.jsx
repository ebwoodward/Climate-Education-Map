...
const columnSymbology = {
  "requires_anthropogenic_cc": (value) => {
    switch (value) {
      case "N": return "#e0a3a3";
      case "Y": return "#6bbd8d";
...
const App = () => {
 ...
 // Function to generate the legend items based on the selected column
const generateLegend = () => {
  const symbology = columnSymbology[selectedColumn];
  if (!symbology) return null;  // Return nothing if no symbology function exists

  const legendItems = [];
  
  // Determine the possible values for the selected column
  let sampleValues = [];
  let isGradient = false;  // Flag to determine if we need a gradient
  switch (selectedColumn) {
    case "score":
      sampleValues = [1, 2, 3, 4, 5];  // For "score" column, use values 1 to 5
      isGradient = true;  // Set the flag to true for gradient
      break;
    case "requires_anthropogenic_cc":
      sampleValues = ["Y", "N"];  // For "requires_anthropogenic_cc" column, use "Y" and "N"
      break;
    case "language_strength":
      sampleValues = ["0.25", "0.5", "0.75", "1"];  // For "language_strength"
      break;
    case "science_standard_type":
      sampleValues = ["State", "Hybrid", "NGSS"];  // For "science_standard_type"
      break;
  ...
  // If it's a gradient (like "score"), create a gradient bar
  if (isGradient) {
    const gradientStyle = {
      width: "200px",
      height: "20px",
    ...
    // Create the legend items for discrete values
        ...
        <div key={value} style={{ display: "flex", alignItems: "center", marginBottom: "8px" }}>
          <div
            style={{
              width: "20px",
              height: "20px",
              backgroundColor: symbology(value), // Dynamically apply color
              marginRight: "10px",
            }}
          ></div>
          <span>{value}</span>
  ...
  // Calculate the maximum width for the legend based on the selected column
const getMaxLegendWidth = () => {
  if (selectedColumn === "score") {
    return "200px"; // Fixed width for the gradient bar
  }
  ...
  return `${maxLength * 10 + 30}px`; // Adjust width based on the length of the longest value
};

  // Helper function to get sample values based on selected column
  ...
  return (
    <div
      style={{
        backgroundColor: "#f4f7f6", // Background color for the whole page
        ...
        <div style={{ display: "flex", alignItems: "flex-start", marginBottom: "30px" }}>
          ...
          <div
            style={{
              ...
              width: getMaxLegendWidth(), // Dynamically adjust width
            }}
          >
            {generateLegend()}
          </div>
